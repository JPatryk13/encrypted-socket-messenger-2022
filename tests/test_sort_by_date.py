...
class TestSortByDate(unittest.TestCase):
    ...
    def test_sort_by_date_multiple_entries_just_time(self) -> None:
        datetime_key = "foo"
        input_val = [{datetime_key: time(1, 1, int(floor(i / 1000000)), i % 1000000)} for i in range(0, 100000, 3000000)]
        expected = input_val
        random.shuffle(expected)  # randomise order of entries
        actual = SortByDate.sort_by_date(input_val, datetime_key)
        self.assertEqual(expected, actual)

    def test_sort_by_date_multiple_entries_just_date(self) -> None:
        datetime_key = "bar"
        input_val = [{datetime_key: date(2019, int(1 + floor(i / 28)), 1 + i % 28)} for i in range(0, 10, 280)]
        expected = input_val
        random.shuffle(expected)  # randomise order of entries
        actual = SortByDate.sort_by_date(input_val, datetime_key)
        self.assertEqual(expected, actual)
    ...
    def test_sort_by_date_multiple_entries_datetime(self) -> None:
        ...
        input_val = self.datetime_dict_list
        expected = input_val
        random.shuffle(expected)  # randomise order of entries
        actual = SortByDate.sort_by_date(input_val, datetime_key)
        self.assertEqual(expected, actual)
