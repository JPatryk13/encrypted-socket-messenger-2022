...
# List of sockets (clients, servers) for select.select()
...
def receive_message(client_socket):
    ...
    try:
        ...
        message_length = int(message_header.decode("utf-8").strip())  # strip() just for clarity

        # Return an object of message header and message data
        return {"header": message_header, "data": client_socket.recv(message_length)}

    except Exception:
        # If we are here, client closed connection violently, for example by pressing ctrl+c on his script or just lost
        # his connection. socket.close() also invokes socket.shutdown(socket.SHUT_RDWR) what sends information about
        # closing the socket (shutdown read/write) and that's also a cause when we receive an empty message
        return False
...
while True:
    ...
    #   - wlist - sockets for data to be send to (checks if for example buffers are not full and socket is ready to send
    #   some data)
    ...
    # reading, writing, errors = select.select(rlist, wlist, xlist)
    ...
    for notified_socket in read_sockets:
        ...
        # Else existing socket is sending a message
            ...
            # i.e. send to all clients except the one who sent it in
